...
void Game::Init(GameConfig* configuration)
{
    ...
    shaders["macDefault"] = LoadShader(gameConfig->shaderPaths["macDefaultVS"].c_str(), gameConfig->shaderPaths["macDefaultFS"].c_str());
    ...
    // Initialize level.
    // LevelObject(models["column0"], shaders["macDefault"], textures["macDefault"]);
...
void Game::Update(float deltaTime)
{
    if (m_GameState == GameState::Playing)
    {
        ...
        for (auto& gameObject : gameObjects)
        {
            ...
            if (enemy)
            {
                if (enemy->state == GameObjectState::Dead) {
                    ...
                    enemy->Update_(deltaTime, mrAngryCube);
                    continue;
